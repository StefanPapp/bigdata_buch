 ...
 * 
 * Format:
 * [[Jahr, [Längengrad, Breitengrad, Wert, Längengrad, ...]], [Jahr2, [Längengrad, ...]], ...]
 * 
 * z.B.:
...
public class GlobeChart {
	...
	private static Logger log = Logger.getLogger(GlobeChart.class
			.getName());
	...
	public String getJavaScript() {
		...
		for(int i=0; i<this.years.size(); i++) {
			...
			if (i < this.years.size()-1) strYears.append(",");
		}
		...
		for(int i=0; i<this.years.size(); i++) {
			sb.append("<span id=\"year"+this.years.get(i)+"\" class=\"year\">"+this.years.get(i)+"</span>\n");
		}
		sb.append("</div>\n\n");
		sb.append("</td></tr><tr><td>");
		
		// Set image Tag
		sb.append("<div id=\"" + strImageTagID + "\" class=\"globe\" style=\"width:"+this.width+"px; height:"+(this.height - 120)+"px;\"></div>\n");
		...
		sb.append("\t\tvar container = document.getElementById('"+strImageTagID+"');\n");
